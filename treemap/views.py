...
def result_map(request):
    ...
    latest_trees = Tree.objects.filter(present=True).exclude(last_updated_by=1).order_by("-last_updated")[0:3]
...
def contact(request):
    if request.method == 'POST': 
        ...
        if form.is_valid():
            ...
            sender = settings.CONTACT_FROM_EMAIL #'no-reply@ashevilletreemap.org' #settings.DEFAULT_FROM_EMAIL
