...
choices_choices = ( 
    ...
    ('tree_owner', 'Owner'),
    ('condition', 'Condition'),
    ('powerline_conflict_potential', 'Powerline Conflict Potential'),
    ('bool_set', 'Bool Set')
)
watch_choices = {
...
#    owner_geometry = models.PointField(null=True, srid=4326) #should we keep this?
    ...
    tree_owner = models.CharField(max_length=256, null=True, blank=True, choices=Choices().get_field_choices('tree_owner'))
    steward_name = models.CharField(max_length=256, null=True, blank=True, choices=Choices().get_field_choices('steward_name'))
    ...
    def get_steward_display(self):
        for key, value in Choices().get_field_choices('steward_name'):
            if key == self.steward_name:
                return value
        return None

    def get_owner_display(self):
        for key, value in Choices().get_field_choices('tree_owner'):
            if key == self.tree_owner:
                return value
        return None
    ...
    def validate_max_dbh(self):
        ...
        if self.dbh > self.species.v_max_dbh:
            return str(self.dbh) + " (species max: " + str(self.species.v_max_dbh) + ")"
        return None
    ...
    def validate_max_height(self):
        ...
        if self.height > self.species.v_max_height:
            return str(self.height) + " (species max: " + str(self.species.v_max_height) + ")"
        return None
